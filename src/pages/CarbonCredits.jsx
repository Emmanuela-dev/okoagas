...
const Counter = ({ value, suffix = '', duration = 2 }) => {
  ...
  return (
    <span ref={ref}>
      {numericValue % 1 === 0 ? Math.floor(displayValue).toLocaleString() : displayValue.toFixed(1)}{suffix}
    </span>
  );
};
  ...
  <PageWrapper>
    ...
    <section className="section text-white text-center" style={{ padding: '120px 0', backgroundColor: '#065f46' }}>
      <div className="container">
        ...
        >
          <Leaf className="w-16 h-16 mx-auto mb-6 text-white" />
          <h1 className="text-white mb-4" style={{ fontSize: '3.5rem' }}>Carbon <span style={{ opacity: 0.8 }}>Credits</span></h1>
          <p className="text-xl mx-auto max-w-2xl" style={{ opacity: 0.9 }}>
    ...
    <section className="section-sm">
      <div className="container">
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} className="grid md:grid-cols-3 gap-6">
          {[
            { icon: <Globe className="text-primary w-8 h-8" />, value: '2.5 Tonnes', label: 'CO2 Saved per Household/Year' },
            { icon: <TrendingUp className="text-primary w-8 h-8" />, value: '15,000+', label: 'Total Credits Generated' },
            { icon: <Award className="text-primary w-8 h-8" />, value: 'Gold Standard', label: 'Certified Methodology' }
          ].map((stat, idx) => (
            <motion.div key={idx} variants={fadeUp} className="glass" style={{ padding: '24px', textAlign: 'center', borderRadius: '20px' }}>
              <div className="mb-3 flex justify-center">{stat.icon}</div>
              <h2 className="mb-1" style={{ fontSize: '1.75rem' }}>
                <Counter value={stat.value.replace(' Tonnes', '').replace('+', '')} suffix={stat.value.includes('Tonnes') ? ' Tonnes' : '+'} />
              </h2>
              <p className="text-muted mb-0" style={{ fontSize: '0.85rem', fontWeight: '700' }}>{stat.label}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
      ...
      <div className="container">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
            <motion.h2 variants={fadeUp} className="mb-8" style={{ fontSize: '2.25rem' }}>How You Earn <span className="text-primary">Credits</span></motion.h2>
            <div className="flex flex-col gap-6">
              {[
                ...
                <motion.div key={idx} variants={fadeUp} className="flex gap-4">
                  <CheckCircle className="text-primary flex-shrink-0 w-6 h-6" />
                  <div>
